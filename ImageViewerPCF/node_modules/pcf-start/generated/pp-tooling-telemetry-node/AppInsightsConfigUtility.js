...
function configureTelemetryClient(aiClient, productName, productVersion, sessionId, environment, userSettings) {
    ...
    if (!(0, AppTelemetryUtility_1.isSupportedAgentProductVersion)(productVersion))
        throw new Error("productVersion '".concat(productVersion, "' is not a supported version format."));
    if (!sessionId)
        ...
        case appInsights.Contracts.TelemetryType.Request:
            {
                ...
                logger.info("[pp-tooling-telemetry-node] Processing telemetry item of type '".concat(envelope.data.baseType, "'. name: ").concat(data.name, ", responseCode: ").concat(data.responseCode));
            }
            break;
        case appInsights.Contracts.TelemetryType.Event:
            {
                var data = envelope.data.baseData;
                logger.info("[pp-tooling-telemetry-node] Processing telemetry item of type '".concat(envelope.data.baseType, "'. name: ").concat(data.name));
            }
            break;
        case appInsights.Contracts.TelemetryType.Exception:
            {
                var data = envelope.data.baseData;
                logger.info("[pp-tooling-telemetry-node] Processing telemetry item of type '".concat(envelope.data.baseType, "'. problemId: '").concat(data.problemId, "'"));
            }
            break;
        default:
            logger.info("[pp-tooling-telemetry-node] Processing telemetry item of type '".concat(envelope.data.baseType, "'"));
            break;
    }
    return true;
}
