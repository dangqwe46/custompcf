...
function getAppDataPath() {
    ...
    switch (platform) {
        case 'darwin':
            return '~/Library/';
        case 'linux':
            return "".concat(process.env.HOME, "/.config/");
        case 'win32':
            return process.env.LOCALAPPDATA;
        default:
            throw new Error("Platform \"".concat(platform, "\" is not currently supported"));
    }
}
function getCurrentUserSettingsOrDefault(provider, logger) {
    try {
        ...
        logger === null || logger === void 0 ? void 0 : logger.error("[pp-tooling-telemetry-node]: ITelemetryUserSettingsProvider.GetCurrent threw unexpected error. Returning new user settings. Error: ".concat(error));
        return {
            uniqueId: uuid.v4(),
