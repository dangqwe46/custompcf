...
function composeAutomationAgentString(productName, productVersion) {
    ...
    if (!ProductNameRegex.test(productName)) {
        throw new Error("productName '".concat(productName, "' is invalid; only the following characters are allowed: letters, numbers, and the symbols '.', '+', '_', '-'."));
    }
    if (!isSupportedAgentProductVersion(productVersion)) {
        throw new Error("productVersion '".concat(productVersion, "' is invalid; it should be a valid System.Version or SemVer (v2)."));
    }
...
function tryParseAutomationAgentStringProductNameVersion(agent) {
    ...
    if (firstProduct.length > MaxAutomationAgentLength) {
        return {
            success: false,
            invalidReason: "The first product (name and version) has a length of ".concat(firstProduct.length, " which is longer than allowed length of ").concat(MaxAutomationAgentLength, "."),
        };
    }
...
function validateAndSanitizeAutomationAgent(environment) {
    ...
    if (result.success !== true) {
        ...
        throw new Error("[AppTelemetryConfigurationException] The automationAgent '".concat(environment.automationAgent, "' is invalid. ").concat(result.invalidReason, " This exception should be considered a developer bug of the calling agent."));
    }
